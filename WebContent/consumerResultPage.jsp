...
<script type="text/javascript">
	...
	function button_event() {
		...
		if (confirm("정말 구매 확정하시겠습니까?? 취소할 수 없습니다") == true) { //확인
			form1.submit();
		} else { //취소
			return;
		}
...
<body>
	...
	<div class="container" align="center">
		<form action="confirmation.do" method="post" name="frm">
			<div class="consumerResultPage">
				<h3 class="sort_type">게시물 > ${b_title }</h3>
				<br> <br>
				<div class="sort_type" style="text-align: center;">
					등록번호 [ ${b_num} ] <input type="hidden" name="b_num"
						value="${b_num }">에
					<c:if test="${seller_user_info.seller_phone != '0' }">낙찰되셨습니다.</c:if>
					<c:if test="${seller_user_info.seller_phone == '0' }">낙찰되지 않으셨습니다.</c:if>
				</div>
				<br> <br>
						...
						<td style="vertical-align: top;">
									...
									<c:forEach var="bid" items="${list }">
										<tr>
											...
											<td><fmt:formatNumber value="${bid.bid_price }"
													pattern="#,###" /></td>
											<c:if test="${bid.bid_price == max_bid_price }">
											</c:if>
										</tr>
										<c:set var="startNum" value="${startNum - 1}" />
									</c:forEach>
									<input type="hidden" name="bid_num_maxBid_price" value="${bid_num_maxBid_price}">
									<input type="hidden" name="max_bid_price" value="${max_bid_price}">
								</c:if>
								<c:if test="${totCnt == 0 }">
									<tr>
							...
							<div style="text-align: center;">
								<c:if test="${startPage > blockSize }">
									<a
										href='consumerResultPage.do?b_num=${b_num }&pageNum=${startPage-blockSize }'>[이전]</a>
								</c:if>
								<c:forEach var="i" begin="${startPage }" end="${endPage }">
									<a href='consumerResultPage.do?b_num=${b_num }&pageNum=${i}'>[${i}]</a>
								</c:forEach>
								<c:if test="${endPage < pageCnt }">
									<a
										href='consumerResultPage.do?b_num=${b_num }&pageNum=${startPage+blockSize }'>[다음]</a>
								</c:if>
							</div>
						</td>
					</tr>
					<tr>
						...
						<td>
							<table class="table-bordered" style="width: 100%;">
								<tr>
									<th colspan="2" align="center">판매자 정보 <%-- <div align="center">
											<img class="img-thumbnail" src="image/profile_image/${seller_user_info.seller_image }"
								...
								<tr>
									<td>판매자 id</td>
									<td>${seller_user_info.seller_email}<input type="hidden"
										name="seller_email" value="${seller_user_info.seller_email}">
									</td>
								</tr>
								<tr>
									<td>판매자 전화번호</td>
									<td><c:if test="${seller_user_info.seller_phone != '0' }">${seller_user_info.seller_phone }</c:if>
										<c:if test="${seller_user_info.seller_phone == '0' }">조회할 권한이 없습니다.</c:if></td>
								</tr>
							</table>
						</td>
						<td style="vertical-align: middle;">
							<div>
								<c:if test="${seller_user_info.seller_phone != '0' }">
									<c:if test="${confirm_status==1}">구매 확정 되었습니다.</c:if>
									<c:if test="${confirm_status==-1}">신고된 내용을 처리중입니다.</c:if>
									<c:if test="${confirm_status==0}">
										<p id="countdown"></p>
										<input type="button" class="btn btn-primary"
											style="background-color: #000000; border-color: #A2A2A2;"
											value="구매확정" onclick="button_event();">
									</c:if>
								</c:if>
								<c:if test="${seller_user_info.seller_phone == '0' }"></c:if>
							</div> <br>
							<div align="center">
								<input type="button" class="btn btn-primary"
									style="background-color: #000000; border-color: #A2A2A2;"
